...
export default function CategoriaForm({ 
  categoriaId,     // para la página
  categoria,       // para el modal
  ...
  useEffect(() => {
    const cargar = async () => {
      // Si estamos en modal y tenemos categoria
      if (esModal && categoria) {
        setFormData({ ...categoria });
        return;
      }

      // Si estamos en página y tenemos categoriaId
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      if (esEdicion) {
        ...
        respuesta = await actualizarCategoria(id, formData);
      } else {
        respuesta = await crearCategoria(formData);
      }
      ...
      if (esModal) {
        ...
        setToastMessage(esEdicion ? "¡Categoría actualizada con éxito!" : "¡Categoría registrada con éxito!");
    ...
    <div className={esModal ? "" : "container py-4"}>
      ...
      {!esModal && (
        <div className="d-flex justify-content-between align-items-center mb-4">
          <h2 className="fw-bold text-primary">{esEdicion ? 'Editar Categoría' : 'Nueva Categoría'}</h2>
...
<div className="card shadow-sm border-0 mb-4">
  <div className="card-header bg-white py-3">
    <h5 className="mb-0 text-secondary">
      <i className="bi bi-folder me-2"></i>Información de la Categoría
    </h5>
  </div>
  <div className="card-body">
    <div className="row g-3">
      ...
      {/* 👇 SECCIÓN DE ACTIVO/INACTIVO - CORREGIDA */}
      <div className="col-md-12">
          ...
          <div className="d-flex align-items-center">
            ...
            <div className="ms-3">
              <label className="form-check-label fw-semibold d-block" htmlFor="activoSwitch" style={{ cursor: "pointer" }}>
                Categoría {formData.activo ? 'Activa' : 'Inactiva'}
              </label>
              <small className="text-muted">
                {formData.activo 
                  ? 'La categoría está habilitada y visible en el sistema' 
                  : 'La categoría está deshabilitada y no estará disponible'}
              </small>
            </div>
          </div>
        </div>
